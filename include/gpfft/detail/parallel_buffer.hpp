...
void parallel_buff_3D<T>::get_slice(int writer, T* buf, int pos)
{
    ...
    if (sender != writer)
    {
        ...
        if (writer == com.rank())
            MPI_Recv(buf, Ntot * sizeof(T), MPI_CHAR, sender, 1, com.get_com(),
                     MPI_STATUS_IGNORE);
    }
...
void parallel_buff_3D<double>::all_to_all()
{
    ...
    MPI_Alltoall(sendbuf.get(), Ntot / com.size(), MPI_DOUBLE, recvbuf.get(),
                 Ntot / com.size(), MPI_DOUBLE, com.get_com());
